...
function shortcode_calltoaction($atts, $content = null) {
   global $lambda_meta_data, $post, $theme_options,$metadata;
  ...
  query_posts( $args );
   ...
   extract(shortcode_atts(array(
	...
	if(!empty($buttontext))
	$cta .= '<div class="remove-bottom" style="float:left;">';

	$blog_posts = query_posts($args);
		
	$cta .= '<h3>'.$headline.'</h3>';
	if(!empty($buttontext))
	$cta .= '<div class="cta-button-wrap remove-bottom one_third last"><a href="'.$buttonlink.'" class="button themebutton medium" target="'.$target.'">'.$buttontext.'</a></div>';
	
	if(!empty($buttontext))
	$cta .= '</div>';
	
   
	  $ctab ='<div class="test" style="float:right;width:50%;">'; 
    $ctab.='<h1>Latest news </h1>';
	foreach($blog_posts as $blog_post)
	{
		
  
	$ctab .= '<span class="test"><h3>'.$blog_post->post_name.'</h3></span>';
	$ctab .= '<span class="expert">'.wp_trim_words($blog_post->post_content,20).'<br> <a href="'. $blog_post->guid .'"> Read More ...</a></span><br>';

	
	
    }
    $ctab .= '</div>';
	$ctab .= '</article></div></section>';
	return $cta.$ctab;
}
